...
psi_bp = Blueprint('psychology', __name__)

@psi_bp.route('/citas', methods=['POST'])
def register_appointment():
    try:
        ...
        cur.execute("INSERT INTO citas_psicologia (estudiante_id, motivo, fecha, hora, sede) VALUES (%s, %s, %s, %s, %s)", 
...
@psi_bp.route('/registrar_recomendacion_ps', methods=['POST'])
def register_recom():
    try:
        data = request.json
        cur = mysql.connection.cursor()
        cur.execute("SELECT id FROM user WHERE identificacion = %s", (data['id_psicologo'],))
        prof = cur.fetchone()
        ...
        cur.execute("INSERT INTO recomendaciones_psicologia (estudiante_id, psicologo_id, recomendacion) VALUES (%s, %s, %s)", 
                    (est[0], prof[0], data['recomendacion']))
        
        # Eliminar cita
        cur.execute("DELETE FROM citas_psicologia WHERE estudiante_id = %s AND estado = 'Programada' LIMIT 1", (est[0],))
